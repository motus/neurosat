...
for fname in sys.argv[1:]:
    ...
    with open(fname) as fh_input:
        for line in fh_input:
            ...
            if not is_valid:
                sat_invalid[n_vars].add(key)

sys.stderr.write("\n")

print("n_vars,sat,sat_invalid,sat_good,sat_good_pct")
for n_vars in [20, 40, 60, 80]:
    n_sat = len(sat_all[n_vars])
    n_invalid = len(sat_invalid[n_vars])
    n_solved = len(sat_solved[n_vars])
    print("%d,%d,%d,%d,%.4f" % (n_vars, n_sat, n_invalid, n_solved, n_solved * 100.0 / n_sat))
